...
describe('block', () => {
  it('parses a block', () => {
    ...
    expect(parser.errors).toHaveLength(0);
    expect(cst.name).toBe('block');
    expect(cst.children['triplesOrGraph']).toHaveLength(1);
    expect(Object.keys(cst.children)).toHaveLength(1);
  });
});
describe('triplesOrGraph', () => {
  it('parses a triplesOrGraph', () => {
    const cst = parse(
      `<http://example/G> { <http://example/a> <http://example/b> <http://example/c> . }`,
      parser.triplesOrGraph
    );
    expect(parser.errors).toHaveLength(0);
    expect(cst.name).toBe('triplesOrGraph');
    expect(cst.children['labelOrSubject']).toHaveLength(1);
    expect(cst.children['wrappedGraph']).toHaveLength(1);
    expect(Object.keys(cst.children)).toHaveLength(2);
  });
});
describe('triples2', () => {
  it('parses a triples2', () => {
    const cst = parse(
      `[a()] <http://example/b> <http://example/c> .`,
      parser.triples2
    );
    expect(parser.errors).toHaveLength(0);
    expect(cst.name).toBe('triples2');
...
describe('wrappedGraph', () => {
  it('parses a wrappedGraph', () => {
    const cst = parse(
      `{ <http://example/a> <http://example/b> <http://example/c> . }`,
      parser.wrappedGraph
    );
    expect(parser.errors).toHaveLength(0);
    expect(cst.name).toBe('wrappedGraph');
    expect(cst.children['triplesBlock']).toHaveLength(1);
    expect(Object.keys(cst.children)).toHaveLength(3);
  });
});
describe('triplesBlock', () => {
  it('parses a triplesBlock', () => {
    const cst = parse(
      `<http://example/a> <http://example/b> <http://example/c> .`,
      parser.triplesBlock
    );
    expect(parser.errors).toHaveLength(0);
    expect(cst.name).toBe('triplesBlock');
    expect(cst.name).toBe('triplesBlock');
    expect(cst.children['triples']).toHaveLength(1);
    expect(Object.keys(cst.children)).toHaveLength(2);
  });
});
describe('labelOrSubject', () => {
  it('parses a labelOrSubject', () => {
    const cst = parse(`<http://example/G>`, parser.labelOrSubject);
    expect(parser.errors).toHaveLength(0);
    expect(cst.name).toBe('labelOrSubject');
    expect(cst.children['iri']).toHaveLength(1);
    expect(Object.keys(cst.children)).toHaveLength(1);
  });
});

// from Turtle lexer tests
...
describe('iri', () => {
  ...
  it('parses an IRIREF that has a UCHAR in it', () => {
    const cst = parse('<http://www.w3.org/2013/TrigTests/\u0021>', parser.iri);
    expect(parser.errors).toHaveLength(0);
    expect(cst.name).toBe('iri');
    expect(cst.children['IRIREF']).toHaveLength(1);
    expect(Object.keys(cst.children)).toHaveLength(1);
